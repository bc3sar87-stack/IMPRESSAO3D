...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ codigo: string }> }) {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE clientes SET tipo_pessoa=$1, documento=$2, nome=$3, telefone=$4, email=$5, endereco=$6
       WHERE codigo=$7 AND empresa_codigo=$8
       RETURNING codigo`,
      [tipo_pessoa, documento, nome, telefone || null, email || null, endereco || null, codigo, session.empresa_codigo]
    );
    if (rows.length === 0) {
